...
use crate::netbuilder::{ComponentType, NetworkComponentBuilder};
...
fn main() {
    println!("Sample Network Code");

    let mut simulation = Simulation::<DelayChannelBuilder>::default();
    let mut component_builder = NetworkComponentBuilder::new(ComponentType::Host);
